...
def main(smk: Any, sconf: cfg.GiabStrats) -> None:
    ...
    p = sconf.to_build_pair(ws["ref_key"], ws["build_key"])
    hap = cfg.to_haplotype(ws["hap"])

    if isinstance(p.ref, cfg.HaploidRefKey):
        # one haplotype for both bed and ref (no combine)
        match ins:
            case [i]:
                rm = sconf.haploid_stratifications[
                    p.ref
                ].strat_inputs.low_complexity.rmsk
                assert rm is not None, "this shouldn't happen"
                read_filter_sort_hap_bed(
                    sconf, i, o, rm.params, p, rm.data.chr_pattern, [rm.class_col]
                )
            case _:
                assert False, "this should not happen"

    elif isinstance(p.ref, cfg.Diploid1RefKey):
        # one bed with both haps in it; one reference with both haps (no combine)
        rm0 = sconf.diploid1_stratifications[p.ref].strat_inputs.low_complexity.rmsk
