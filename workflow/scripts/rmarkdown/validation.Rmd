...
setwd("/Users/jmcdani/Documents/GiaB/Benchmarking/Stratifications/v3.1_genome-stratifications/validation")
```
---
### Purpose
...
library(tidyverse)
library(here)
# TODO replace with directory from snakemake input holding all bed files
beds38 <- list.files(here("v3.1-candidate-release-files/GRCh38"),
                     full.names = TRUE,
                     pattern = ".bed.gz",
                     recursive = TRUE)
## beds37 <- list.files(here("v3.1-candidate-release-files/GRCh37"),
##                      full.names = TRUE,
##                      pattern = ".bed.gz",
##                      recursive = TRUE)
## beds13 <- list.files(here("v3.1-candidate-release-files/CHM13v2.0"),
##                      full.names = TRUE,
##                      pattern = ".bed.gz",
##                      recursive = TRUE)
...
## beds13_df <- map_dfr(.x= beds13_set,
##                      read_tsv, 
##                      skip =1,
##                      col_types = "cdd",
##                      col_names= c("chrom","start","end"),
...
## strat37_summary <- here("v3.1-stratfications-info.csv") %>%
##   read.csv(header = TRUE, na.strings = TRUE) %>% 
##   filter(reference =="GRCh37")

## strat13_summary <- here("v3.1-stratfications-info.csv") %>%
##   read.csv(header = TRUE, na.strings = TRUE) %>% 
##   filter(reference =="CHM13")

beds38_df_with_levels <- left_join(beds38_df, strat38_summary) 

## beds37_df_with_levels <- left_join(beds37_df,strat37_summary) 

## beds13_df_with_levels <- left_join(beds13_df,strat13_summary)
...
## expected_chroms_13 <- beds13_df_with_levels %>%
##   filter(chrom %in% expected_chroms)
## unexpected_chroms_13 <- beds13_df_with_levels %>%
##   filter(!chrom %in% expected_chroms)
...
## chromregions_37 <- read_tsv("genome-bed-files/GRCh37_1-22XY_sorted_NoNs_NoPSAY_merged.genome.bed",
##                             col_names = cols) %>%
##   mutate(region_size = end - start)

# TODO what is this file?
chromregions_38 <- read_tsv("genome-bed-files/GRCh38_1-22XY_sorted_NoNs_NoPSAY_merged.genome.bed",
                            col_names = cols)%>%
  mutate(region_size = end - start)

## chromregions_13 <- read_tsv("genome-bed-files/CHM13v2.0_1-22XY_sorted.genome.bed",
##                             col_names = cols)%>%
##   mutate(region_size = end - start)
...
## beds13_df_with_levels <- mutate(beds13_df_with_levels,
##                                               region_size = end - start)
...
## chrom_region_sizes_13stratifications <- beds13_df_with_levels %>%
##   group_by(stratification_filename, chrom) %>%
##   summarise(total_region = sum(region_size))
...
## stratANDref_13chromsizes <- left_join(chrom_region_sizes_13stratifications,
##                                       chromsizes_13)
## stratANDref_13chromsizes <- add_column(stratANDref_13chromsizes,
##                                        "reference" = "CHM13")
