...
chr_mapper <- readr::read_tsv(
  ...
  col_names = c("chrom_index", "build_key", "haplotype", "chrom"),
  col_types = "iccc"
) %>%
...
chr_build_combinations <- chr_mapper %>%
  expand(chrom_index, nesting(build_key,  haplotype))
...
format_key <- function(build_key, haplotype) {
  ms <- str_match(build_key, "(.+?)(\\.hap(1|2))?@(.+)")
  sprintf("%s@%s-%s", ms[,2], ms[,5], haplotype)
}
...
read_strats <- function(stype, gaps) {
  ...
  if (length(fs) > 0) {
    ...
    all_combos <- chr_build_combinations %>%
      expand(nesting(chrom_index, build_key, haplotype), strat_name = df$strat_name)
    df %>%
      ...
      # zero coverage
      full_join(all_combos, by = c("chrom_index", "strat_name", "build_key", "haplotype")) %>%
      left_join(df_chr_sizes, by = c("chrom_index", "build_key")) %>%
      mutate(strat_type = stype,
             key = format_key(build_key, haplotype),
